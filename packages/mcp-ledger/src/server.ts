...
import { z } from 'zod';
import { defaultTaxConfig, type TaxConfig } from '@hisab/shared';
import type { Db } from '@hisab/db';
import { createToolHandlers, inputSchemas, toolDescriptions, type ToolContext } from './tools.js';
...
export function buildLedgerServer(deps: LedgerDeps, tenantId: string): McpServer {
  const server = new McpServer({ name: 'hisab-ledger', version: '0.1.0' });
  const ctx: ToolContext = { db: deps.db, tenantId, cfg: deps.cfg ?? defaultTaxConfig };
